...
import {
    Printer, ArrowRight, CheckCircle2, BookOpen, Package, Layers,
    Droplets, BookType, Box, Truck, Globe, Phone, Minus, Plus,
    Sparkles, PenTool, ShieldCheck, Scale, Search, FileCheck, X
} from "lucide-react";
...
const printMethods = [
    {
        ...
        subtitle: "No Inventory Risk",
        icon: Package,
        features: [
            { text: "Zero upfront inventory costs", good: true },
            { text: "Books printed one at a time as ordered", good: true },
            { text: "Global printing & local shipping", good: true },
            { text: "Higher per-unit cost", good: false },
            { text: "Ideal for new & mid-list authors", good: true },
        ],
        tag: "Most Popular"
    },
    {
        title: "Offset Printing",
        subtitle: "Bulk Volume Savings",
        icon: Printer,
        features: [
            { text: "Significantly lower per-unit cost", good: true },
            { text: "Premium print & paper quality", good: true },
            { text: "Custom finishes & embellishments", good: true },
            { text: "Requires minimum order (500+)", good: false },
            { text: "Ideal for events & bulk sales", good: true },
        ],
        tag: "Best Value"
    },
];
...
const printFinishes = [
    { icon: Layers, title: "Paper Stock", desc: "Choose from premium cream or white interior stock (50lb–80lb). Uncoated for novels, coated for photography and art books." },
    { icon: BookType, title: "Binding Options", desc: "Perfect binding for paperbacks, case binding for hardcovers, saddle-stitch for booklets, and spiral for workbooks." },
    { icon: Droplets, title: "Cover Finishes", desc: "Gloss lamination for vibrant pop, matte for sophisticated elegance, or soft-touch velvet for a premium tactile feel." },
    { icon: Sparkles, title: "Special Embellishments", desc: "Foil stamping (gold, silver, holographic), embossing, debossing, and spot UV coating for luxury editions." },
];

const processSteps = [
    { step: "01", title: "Specifications & Quote", desc: "We determine your ideal trim size, paper stock, binding, and finish — then provide a transparent, itemized quote.", icon: Scale },
    { step: "02", title: "File Pre-Flight", desc: "Our pre-press team inspects your files for bleed, resolution, color profiles, and margin compliance to prevent printing errors.", icon: FileCheck },
    { step: "03", title: "Physical Proof", desc: "You receive a physical proof to hold, review, and approve before the full print run begins. What you see is what you get.", icon: Search },
    { step: "04", title: "Production & Fulfillment", desc: "Your books are printed, quality-checked, and shipped directly to you, your warehouse, or distributed globally.", icon: Truck },
];

const faqs = [
    { q: "What is the minimum order quantity?", a: "For Print-on-Demand, there is no minimum — books are printed as they're ordered. For offset printing, the minimum is typically 500 copies to achieve meaningful per-unit savings." },
    { q: "How long does book printing take?", a: "POD orders print and ship within 3–5 business days. Offset print runs typically take 10–15 business days from proof approval, plus shipping time." },
    { q: "What file format do you need for printing?", a: "We prefer press-quality PDF files with embedded fonts, CMYK color space, and proper bleed (0.125\" on all sides). Our pre-press team will verify everything before printing." },
    { q: "Can I order author copies at a discount?", a: "Absolutely. Both POD and offset allow you to order author copies at the base manufacturing cost, significantly below retail price. Perfect for signings, events, and direct sales." },
    { q: "What is the difference between matte and gloss cover finish?", a: "Gloss lamination makes colors vibrant and shiny, ideal for illustrated or photography-heavy covers. Matte lamination has a smooth, non-reflective finish that feels sophisticated and resists fingerprints. Soft-touch is a premium matte variant with a velvety texture." },
    { q: "Do you offer hardcover printing?", a: "Yes. We offer both case-wrapped hardcovers with glossy or matte finishes, and dust jacket hardcovers with foil and embossing options. Hardcover is available for offset and select POD networks." },
];
...
export default function BookPrintingPage() {
    ...
    return (
        <main className="w-full overflow-hidden" style={{ fontFamily: "'Raleway', Arial, sans-serif" }}>
            ...
            <section className="relative w-full h-screen flex items-center justify-center bg-[#05070f] overflow-hidden">
                ...
                <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="flex items-center justify-center gap-3 mb-6">
                        <Printer size={16} className="text-[#e8391d]" />
                        <span className="text-[#e8391d] font-black uppercase tracking-[0.28em] text-[11px]">Book Printing Services</span>
                    </motion.div>
                    ...
                    <motion.h1 variants={maskReveal} initial="hidden" animate="visible" className="font-black text-white uppercase leading-[0.95] mb-8" style={{ fontSize: "clamp(3rem, 8vw, 7rem)" }}>
                        BUILT TO BE <br /><span className="text-[#e8391d]">HELD.</span>
                    </motion.h1>
                    ...
                    <motion.p variants={fadeUp} initial="hidden" animate="visible" className="text-white/60 leading-[1.85] max-w-2xl mx-auto mb-10" style={{ fontSize: "clamp(0.9rem, 1.1vw, 1.05rem)" }}>
                        There's nothing like holding your finished book for the first time. We deliver professional, bookstore-quality printing — from single-copy print-on-demand to bulk offset runs with premium finishes.
                    </motion.p>
                ...
                <div className="max-w-[1200px] mx-auto px-8 lg:px-16 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                    <motion.div variants={staggerContainer} initial="hidden" animate={overviewInView ? "visible" : "hidden"} className="flex flex-col">
                        ...
                        <motion.h2 variants={fadeUp} className="font-black text-black uppercase leading-[1.05] mb-6" style={{ fontSize: "clamp(2rem, 3.8vw, 3.2rem)" }}>
                            BOOKSTORE QUALITY. <br /><span className="text-[#e8391d]">NO COMPROMISES.</span>
                        </motion.h2>

                        <motion.p variants={fadeUp} className="text-gray-500 leading-[1.85] mb-5" style={{ fontSize: "0.95rem" }}>
                            A cheaply printed book destroys your credibility the moment a reader opens it. Thin, see-through pages, cracked spines, and muddy cover colors scream amateur — and reviewers will notice.
                        </motion.p>
                        <motion.p variants={fadeUp} className="text-gray-500 leading-[1.85] mb-8" style={{ fontSize: "0.95rem" }}>
                            We use the same commercial-grade presses and premium materials used by the Big Five publishers. Whether you need one copy or ten thousand, your books will look, feel, and smell like they belong on the bestseller shelf.
                        </motion.p>

                        <motion.div variants={staggerContainer} className="flex flex-col gap-4">
                            {["Premium Paper Stocks (50lb–80lb)", "Hardcover & Paperback Options", "Physical Proof Before Full Run", "Global Fulfillment & Distribution"].map((item) => (
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.9 }} animate={overviewInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.8, ease: smoothEase }} className="relative hidden lg:block">
                        <div className="relative rounded-3xl overflow-hidden shadow-2xl shadow-black/20" style={{ aspectRatio: "4/5" }}>
                            <Image src="/images/services-printing.webp" alt="Book Printing Service" fill className="object-cover" />
            ...
            {/* ════════════════════════════════════════════
                SECTION 3: POD VS. OFFSET (UNIQUE)
            ════════════════════════════════════════════ */}
                ...
                <div className="max-w-[1200px] mx-auto px-8 lg:px-16 relative z-10">
                    <div className="text-center mb-16 overflow-hidden">
                        <motion.div initial={{ y: "100%" }} whileInView={{ y: 0 }} viewport={{ once: true }} className="flex items-center justify-center gap-3 mb-4">
                            ...
                            <span className="text-[#e8391d] font-black uppercase tracking-[0.28em] text-[11px]">Two Methods</span>
                        </motion.div>
                        <motion.h2 variants={maskReveal} initial="hidden" whileInView="visible" viewport={{ once: true }} className="font-black text-white uppercase leading-none" style={{ fontSize: "clamp(2.5rem, 5vw, 4rem)" }}>
                            POD VS. OFFSET <span className="text-[#e8391d]">PRINTING</span>
                        </motion.h2>
                    </div>
                    ...
                    <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {printMethods.map(({ title, subtitle, icon: Icon, features, tag }) => (
                            <motion.div key={title} variants={fadeUp} className="bg-white/[0.02] border border-white/10 rounded-2xl p-8 hover:border-[#e8391d]/50 transition-all duration-500 relative overflow-hidden">
                                {/* Tag */}
                                <div className="absolute top-4 right-4">
                                    <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${tag === "Most Popular" ? "bg-[#e8391d] text-white" : "bg-white/10 text-white/30"
                                        }`}>
                                        {tag}
                                    </span>
                                </div>
                                ...
                                <div className="flex items-center gap-4 mb-6">
                                    <div className="w-14 h-14 rounded-xl bg-[#e8391d]/10 flex items-center justify-center">
                                        <Icon size={26} className="text-[#e8391d]" />
                                    </div>
                                    <div>
            ...
            {/* ════════════════════════════════════════════
                SECTION 4: PREMIUM PRINT FINISHES (UNIQUE)
            ════════════════════════════════════════════ */}
            <section className="relative w-full bg-[#faf9f7] py-32 overflow-hidden">
                <div className="max-w-[1200px] mx-auto px-8 lg:px-16">
                    <div className="text-center mb-16">
                        <motion.div initial={{ y: "100%" }} whileInView={{ y: 0 }} viewport={{ once: true }} className="flex items-center justify-center gap-3 mb-4">
                            <Box size={16} className="text-[#e8391d]" />
                            <span className="text-[#e8391d] font-black uppercase tracking-[0.28em] text-[11px]">Craftsmanship</span>
                        </motion.div>
                        <motion.h2 variants={maskReveal} initial="hidden" whileInView="visible" viewport={{ once: true }} className="font-black text-black uppercase leading-none" style={{ fontSize: "clamp(2.5rem, 5vw, 4rem)" }}>
                            PREMIUM PRINT <span className="text-[#e8391d]">FINISHES</span>
                        </motion.h2>
                        <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="text-gray-400 mt-4 max-w-xl mx-auto text-[15px] leading-relaxed">
                            The tactile experience of your book matters. Choose from professional finishes that elevate your book from standard to luxury.
                        </motion.p>
                    </div>

                    <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {printFinishes.map(({ icon: Icon, title, desc }) => (
            ...
            {/* ════════════════════════════════════════════
                SECTION 5: PRINTING PROCESS
            ════════════════════════════════════════════ */}
            <section className="relative w-full bg-[#111] py-32 overflow-hidden">
                <div className="max-w-[1200px] mx-auto px-8 lg:px-16 relative z-10">
                    <div className="text-center mb-20">
                        <motion.h2 variants={maskReveal} initial="hidden" whileInView="visible" viewport={{ once: true }} className="font-black text-white uppercase leading-none" style={{ fontSize: "clamp(2.5rem, 5vw, 4rem)" }}>
                            OUR PRINTING <span className="text-[#e8391d]">PROCESS</span>
                        </motion.h2>
                    </div>
            ...
            {/* ════════════════════════════════════════════
                SECTION 6: WHY CHOOSE OUR PRINTING
            ════════════════════════════════════════════ */}
            <section className="relative w-full bg-[#faf9f7] py-32 overflow-hidden">
                <div className="max-w-[1200px] mx-auto px-8 lg:px-16 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                    <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative">
                        <div className="rounded-3xl overflow-hidden shadow-2xl" style={{ aspectRatio: "1/1" }}>
                            <Image src="/images/about-us-bg.webp" alt="Book Printing Quality" fill className="object-cover" />
                        ...
                        <div className="absolute -right-6 top-12 bg-[#e8391d] rounded-2xl px-6 py-4 shadow-2xl shadow-[#e8391d]/30">
                            ...
                            <p className="text-white/70 text-[11px] font-bold uppercase tracking-wider">Misprints tolerated</p>
                        </div>
                    </motion.div>
                    ...
                    <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                        ...
                        <motion.h2 variants={fadeUp} className="font-black text-black uppercase leading-[1.05] mb-10" style={{ fontSize: "clamp(2rem, 3.5vw, 3rem)" }}>
                            QUALITY YOU CAN <br /><span className="text-[#e8391d]">FEEL IN YOUR HANDS.</span>
                        </motion.h2>

                        <motion.div variants={staggerContainer} className="flex flex-col gap-5">
                            {[
                                "Big Five Quality — We use the same commercial presses and materials as major publishers.",
                                "Physical Proof Approval — You hold and approve a proof before we print a single copy.",
                                "Rigorous Pre-Flight Checks — Our technicians inspect every file to prevent costly printing errors.",
                                "No Inventory Risk — POD means zero upfront storage. Offset means bulk savings when you need it.",
                                "Global Fulfillment — Print locally, ship globally. Your books reach readers faster and cheaper."
                            ].map((item) => (
                ...
                <div className="max-w-[900px] mx-auto px-8 relative z-10">
                    <div className="text-center mb-16">
                        <motion.h2 variants={maskReveal} initial="hidden" whileInView="visible" viewport={{ once: true }} className="font-black text-white uppercase leading-none" style={{ fontSize: "clamp(2rem, 4vw, 3rem)" }}>
                            BOOK PRINTING <span className="text-[#e8391d]">FAQS</span>
                        </motion.h2>
                    </div>
            ...
            <section className="relative w-full bg-[#e8391d] py-28 overflow-hidden">
                ...
                <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-4xl mx-auto text-center px-8 relative z-10">
                    ...
                    <p className="text-white/80 text-lg max-w-xl mx-auto mb-10 leading-relaxed">
                        Get a custom print quote based on your exact specifications. No surprises, no hidden fees — just bookstore-quality books.
                    </p>
                    <div className="flex flex-wrap justify-center gap-5">
                        <motion.a href="/contact" whileHover={{ backgroundColor: "#fff", color: "#e8391d", gap: "14px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }} whileTap={{ scale: 0.95 }} className="inline-flex items-center gap-3 bg-black text-white font-black uppercase tracking-widest px-10 py-5 rounded-xl text-[14px] cursor-pointer transition-all duration-300">
                            Get A Print Quote <ArrowRight size={18} />
                        </motion.a>
